...
**The mark is two columns brought to one baseline.** The brass plate of four stacked horizontal bars was replaced on 4 September: at the 28px it actually renders, four horizontal lines in a rounded square is indistinguishable from a hamburger menu, and unequal line lengths were far too subtle a correction. Nothing built from a horizontal stack escapes that read, so the form is vertical. It also says what the product does. Stroke weight steps up at the small size, because a weight that is correct at 96px disappears in a browser tab.

**Every colour is now a step on a ramp.** Four scales, anchored on the mark's own two colours: forest (5 steps), brass (5), parchment (4), oxblood (3). The semantic tokens are aliases pointing at steps, so changing a surface is a change of *step* and two adjacent planes are related by construction rather than by eye. The forest ramp had to be re-derived once: the first attempt interpolated toward a mid-tone and produced a 200 step lighter than its 300, so panels layered in the wrong order. Ramp steps are raw material and are not all safe as text. `forest-500` is 1.58:1 on the card and exists for borders.

**The copy was cut and the em dashes are gone.** The landing page went from roughly 400 rendered words to 178, and all five public pages now report **zero** em dashes in rendered text. Stage descriptions became one or two sentences, both explanatory paragraphs went, and the footnote qualifying the match rate was replaced by four words and a link. `/docs` carries the detail for the reader who wants it, which is what that page is for. Em dashes remain in code comments, which are documentation and which no user reads.

**Not done, deliberately:** converting the three stage cards into a single diagram. It was planned, and after the copy cut it stopped being an improvement. The cards are already an icon and one sentence each; replacing them with an SVG would have added visual complexity to a page whose stated problem was that too much was going on.
